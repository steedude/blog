...
function htmlToMarkdown(html) {
  const blocks = [];
  let result = html.replace(/<pre><code>([\s\S]*?)<\/code><\/pre>/g, (_, code) => {
  ...
  result = result
    ...
    .replace(/<[^>]+>/g, "");
  result = decodeEntities(result).replace(/@@BLOCK_(\d+)@@/g, (_, index) => blocks[Number(index)]);
  return result.replace(/[ \t]+\n/g, "\n").replace(/\n{3,}/g, "\n\n").trim();
}

const source = await readFile(new URL("../frontend-topic-plan-2025-2026.html", import.meta.url), "utf8");

for (const [slug, publishedAt, category, categorySlug, tags, description] of plans) {
  const pattern = new RegExp(`<article class="draft" id="${slug}">[\\s\\S]*?<h3>([\\s\\S]*?)<\\/h3>([\\s\\S]*?)<\\/article>`);
...
const imports = plans.map(([slug]) => {
  ...
  return `import ${name}, { metadata as ${name}Metadata } from "@/content/posts/${slug}/zh-TW.mdx";`;
}).join("\n");
const entries = plans.map(([slug]) => {
  const name = componentName(slug);
  return `  createPost(${name}, ${name}Metadata),`;
}).join("\n");
...
console.log(`Generated ${plans.length} Traditional Chinese MDX articles and their registry.`);
