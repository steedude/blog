...
test("search survives reload, language switching, clearing, and browser back", async ({ page }) => {
  ...
  await page.reload();
  await expect(input).toHaveValue("");
  await expect(page.locator("main article")).toHaveCount(28);
});
...
test("table of contents and heading permalinks navigate to real sections", async ({ page }) => {
  await page.goto("/zh-TW/posts/postman-datasets");
...
test("copies code with line breaks and reports clipboard failure", async ({ page, context }) => {
  ...
  await page.goto("/en/posts/custom-select");
  ...
  expect(clipboard).toContain("appearance: base-select;");
  expect(clipboard).not.toContain("Copy code");
  const source = (await readFile("content/posts/custom-select/en.mdx", "utf8")).replace(/\r\n/g, "\n");
...
test("article and search layouts fit the viewport without page overflow", async ({ page }, testInfo) => {
  for (const path of ["/zh-TW/posts/postman-datasets", "/en/search?q=CSS", "/en/projects/web-file"]) {
...
test("article navigation and sources work without JavaScript", async ({ browser }) => {
  ...
  try {
    await page.goto("http://127.0.0.1:3139/en/posts/postman-datasets");
    ...
    await expect(page.getByRole("link", { name: "Introducing Postman Datasets" })).toBeVisible();
  } finally {
    await context.close();
  }
});
