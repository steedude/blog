...
test("removed articles are unavailable and absent from indexes", async () => {
  for (const locale of ["zh-TW", "en"]) {
    ...
    for (const slug of ["nuxt-4","server-components-retrospective","web-vitals","view-transitions","nuxt-4-5","astro-6-beta","astro-7","react-compiler","react2shell","web-platform-baseline","react-foundation","nx-s1ngularity","node-24","typescript-native-port"]) {
    ...
    for (const path of [`/${locale}/search`, `/${locale}/rss.xml`]) {
      ...
      assert.doesNotMatch(html, /modern-css|angular-22|angular-21|angular-20|vite-8-rolldown|typescript-6|server-components-retrospective|web-vitals|view-transitions|nuxt-4|astro-6-beta|astro-7|react-compiler|react2shell|web-platform-baseline|react-foundation|nx-s1ngularity|node-24|typescript-native-port/);
    }
  }
  const sitemap = await render("/sitemap.xml").then((result) => result.text());
  assert.doesNotMatch(sitemap, /modern-css|angular-22|angular-21|angular-20|vite-8-rolldown|typescript-6|server-components-retrospective|web-vitals|view-transitions|nuxt-4|astro-6-beta|astro-7|react-compiler|react2shell|web-platform-baseline|react-foundation|nx-s1ngularity|node-24|typescript-native-port/);
});
