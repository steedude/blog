...
The phrase "cold wallet" sounds reassuring: if the private key never goes online, the assets should be safe. The troubling part of the Bybit incident is that it showed how the device may remain secure while the signing screen seen by a person is not trustworthy.
